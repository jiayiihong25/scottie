...
1. **Is the OmniRoute instance already running somewhere reachable from
   wherever this pipeline runs** (your machine, eventually a Claude Code
   Routine's environment), or does the daily routine need to also start
   it? Self-hosted gateways typically need to already be up.
2. **Auth**: does your OmniRoute instance require a bearer key? If so it
   needs to come from `.env` (already gitignored) via `python-dotenv`,
   same pattern as any other secret in this repo.
3. **Aliases**: what aliases have you already configured in OmniRoute
   (or do you want me to propose a starter `combos`/alias config too)?
4. Confirm the graph shape above (5-6 nodes, one conditional router) is
   the granularity you want before I start writing `graph/`.
